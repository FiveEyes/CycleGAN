...
a_data = image_folder.ImageFolder(dataset_dir + '/trainA', transform=transform)
b_data = image_folder.ImageFolder(dataset_dir + '/trainB', transform=transform)
a_test_data = image_folder.ImageFolder(dataset_dir + '/testA', transform=transform)
b_test_data = image_folder.ImageFolder(dataset_dir + '/testB', transform=transform)
...
""" run """
a_real_test = Variable(iter(a_test_loader).next(), volatile=True)
b_real_test = Variable(iter(b_test_loader).next(), volatile=True)
...
for epoch in range(start_epoch, epochs):
    for i, (a_real, b_real) in enumerate(zip(a_loader, b_loader)):
        ...
        print(g_loss, a_d_loss + b_d_loss)
    ...
    torchvision.utils.save_image(pic, '%s/Epoch_(%d).jpg' % (save_dir, epoch), nrow=3)
